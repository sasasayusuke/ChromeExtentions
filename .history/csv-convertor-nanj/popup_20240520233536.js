...
function getRandomCharacter(excludeNames = [], firstChoice = false) {
    ...
    const filteredCharacters = characters.filter(character => !excludeNames.includes(character.name));

    // 除外キャラクターを取り除いたリストが空でないかチェック
    if (filteredCharacters.length === 0) {
        throw new Error("All characters are excluded. Cannot select a character.");
    }

    // firstChoice が true の場合、firstChoice フラグが設定されたキャラクターを返す
    if (firstChoice) {
        const firstChoiceCharacter = filteredCharacters.find(character => character.firstChoice);
        if (firstChoiceCharacter) {
            return firstChoiceCharacter;
        }
    }
