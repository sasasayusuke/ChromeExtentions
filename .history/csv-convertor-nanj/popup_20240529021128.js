...
document.addEventListener('DOMContentLoaded', function () {
    try {
        ...
        document.getElementById('csvDownload').addEventListener('click', async function () {
            try {
                console.log('click');
                let lines = await getContentsFromActiveTab(() => Array.from(document.getElementsByClassName("t_b")).map(v => ({ "text": v.innerText, "color": v.style.color })));
                let narratorsColor = lines[0].color
                if (!lines) {
